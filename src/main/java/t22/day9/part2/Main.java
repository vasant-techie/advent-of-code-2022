package t22.day9.part2;

import t22.day9.part1.Coord;
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        Main main = new Main();
        main.process("src/main/resources/day9-part2-sample");
    }
    ...
    private void process(String path) throws IOException {
        ...
        calcUniqueVisited(lines, 9);
        System.out.println("##### Result: " + tailCoords.size());
    }

    private void calcUniqueVisited(List<String> lines, int tailCount) {
                    ...
                    for (int i = 1; i <= count; i++) {
                        ...
                        if (headYEndPos == tailYEndPos) {
                            if (diff > 1) {
                                ...
                                int tempXCoord = tailXEndPos;
                                int tempCount = 1;
                                while(tempCount <= tailCount && tempXCoord >= 1) {
                                    tempXCoord--;
                                    if(tempCount == 9)
                                        tailCoords.add(new Coord(tempXCoord, tailYEndPos));
                                    tempCount++;
                                }
                            }
                        } else {
                            if (diff > 1) {
                                ...
                                int tempXCoord = tailXEndPos;
                                int tempCount = 1;
                                while(tempCount <= tailCount && tempXCoord <= this.xCoord) {
                                    if(tempCount == 9)
                                        tailCoords.add(new Coord(tempXCoord, tailYEndPos));
                                    tempCount++;
                                    tempXCoord++;
                ...
                case "U":
                    ...
                    for (int i = 1; i <= count; i++) {
                        ...
                        if (headXEndPos == tailXEndPos) {
                            if (diff > 1) {
                                tailYEndPos = (headYEndPos > tailYEndPos) ? tailYEndPos + 1 : tailYEndPos - 1;
                                int tempYCoord = tailYEndPos;
                                int tempCount = 1;
                                while(tempCount <= tailCount && tempYCoord <= this.yCoord) {
                                    if(tempCount == 9)
                                        tailCoords.add(new Coord(tailXEndPos, tempYCoord));
                                    tempCount++;
                                    tempYCoord++;
                                }
                            }
